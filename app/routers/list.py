...
@router.post("/")
def post_list(list:schemas.ListCreate,db:Session=Depends(get_db)):
...
@router.get("/{id}")
def get_list_by_id(id:int ,db:Session=Depends(get_db)):
...
def update_list(id:int,updated_list:schemas.ListCreate ,db:Session=Depends(get_db)):
